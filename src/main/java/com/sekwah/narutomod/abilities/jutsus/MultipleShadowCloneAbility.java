    ...
    public void performServer(Player player, INinjaData ninjaData, int ticksActive) {
        ninjaData.scheduleDelayedTickEvent((delayedPlayer) -> {
            ...
            for (int i = 0; i < 10; i++) {
                ...
                if (hasBow) {
                    // Dispersion circulaire sur deux anneaux pour 10 clones
                    double angle = (2 * Math.PI / 5) * (i % 5); // 5 positions par anneau
                    double radius = (i < 5) ? 3.0D : 5.0D; // Deux cercles : un petit et un plus grand
